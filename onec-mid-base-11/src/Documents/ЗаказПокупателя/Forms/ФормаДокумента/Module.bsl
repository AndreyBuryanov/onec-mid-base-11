...
Процедура РассчитатьСуммуСтроки(ТекущиеДанные)
	...
	Если КоэффициентСкидки < 0 Тогда
		КоэффициентСкидки = 0;
		ОбщегоНазначенияКлиент.СообщитьПользователю(СтрШаблон("Сумма скидки по позиции ""%1"" превысила 100%% с учетом общей скидки по документу",
...
&НаКлиенте
Процедура Подключаемый_Д_ПересчитатьСуммы(Команда)
	Д_ПересчитатьСуммыТабличныхЧастей(); // ++ Бурьянов 29.04.2024
КонецПроцедуры
...
&НаКлиенте
Процедура Подключаемый_Д_СогласованнаяСкидкаПриИзменении(Элемент)
	Д_ОбработатьИзменениеСкидкиАсинхронно();// ++ Бурьянов 29.04.2024
КонецПроцедуры
